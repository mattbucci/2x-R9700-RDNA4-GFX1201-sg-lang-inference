...
7. **REAM variant coverage for every MoE model** — Coder-Next-REAM 60B proves the pruned-experts path works on our stack (25 tok/s @ 131K).  Produce + bench + validate REAM variants for each remaining MoE: Qwen3-Coder-30B-A3B, Gemma4-26B, Qwen3.5-35B-A3B, Qwen3.6-35B-A3B.  For each: find or produce pruned weights, self-calibrate if the public drop lacks thinking/vision, pass `validate_capabilities.py`, bench full 256K sweep.  Expected wins: lower VRAM → longer context ceiling, faster decode at the same parameter count, clean story for users picking between full vs pruned.
