...
- **Qwen3.6-35B-A3B AWQ CT path ~3-4x slower than GPTQ path** (2026-04-23).  Our self-calibrated AWQ ships as compressed-tensors and SGLang loads it via `CompressedTensorsWNA16TritonMoE`, which measured 3.6 tok/s short-ctx / 3.4 @131K on the R9700 pair — vs Qwen3.5-35B-A3B GPTQ via `moe_wna16` at 14–16 short / 12.4 @256K.  Same hardware, same tensor parallelism.  The compressed-tensors MoE kernel on ROCm likely hasn't gotten the same fused-GEMM tuning as the AWQ `moe_wna16` path.  Workaround for users who don't need thinking+vision: stick with the GPTQ Qwen3.5-35B build.  Fix path: either (a) write a CT→AWQ converter for Qwen3.6-35B so it loads via `moe_wna16`, or (b) bring the ROCm `CompressedTensorsWNA16TritonMoE` kernel up to `moe_wna16` parity.
...
| Qwen3.6-35B MoE AWQ | MoE+DeltaNet (256 experts) | 262K | 3.6 | 3.4 @131K | `launch.sh qwen36-moe` | Working, compressed-tensors path ~3-4x slower than Qwen3.5-35B GPTQ (see Known Issues) |
