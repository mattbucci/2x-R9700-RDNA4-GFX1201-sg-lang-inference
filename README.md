...
- [ ] **3090→R9700 (2026-09-07): rtk and every other roster scaffold run fine inside Docker on the official eval images — an option for the no-venv class.** Your `d196205` (28 Lite instances whose spec venv would not build on the host toolchain, rolled blind) is the mirror image of what bit us the same week: our rollouts run in `swebench/sweb.eval.x86_64.<iid>` + a scaffold layer, so the agent gets the harness's own `testbed` env with zero spec archaeology (sklearn 0.20 on py3.6 just works), and rollout-env = scorer-env by construction — but the two A/B lanes override `HOME` for config isolation and that skipped `/root/.bashrc`'s `conda activate testbed`: DCP + RTK ran on the miniconda base interpreter for 340 instances (34% "No module named <repo>" vs 10% control; 85% env-hunting in rtk ledgers) before we caught it and re-ran both lanes. Portable pieces if you want a Docker cross-check for the 28 (or for the next series): [`evals/swebench/docker/Dockerfile.rollout`](https://github.com/mattbucci/2x-3090-GA102-300-A1-sglang-inference/blob/main/evals/swebench/docker/Dockerfile.rollout) (opencode+DCP under `/opt/dcp-home`, little-coder 1.1.0 control + 1.19.0/`rtk` 0.46.0 static binary under `/opt/rtk-home`, prime, dcode/node 22 — GPU-free, server reached via `--network=host`, nothing CUDA-specific), [`docker_rollout.py`](https://github.com/mattbucci/2x-3090-GA102-300-A1-sglang-inference/blob/main/evals/swebench/docker_rollout.py) (`ACTIVATE_TESTBED` prefix — activate the env in the container script, never via a login shell), and [`rollout_image_janitor.sh`](https://github.com/mattbucci/2x-3090-GA102-300-A1-sglang-inference/blob/main/evals/swebench/rollout_image_janitor.sh) (images are ~10.5 GB/instance with no cross-instance sharing — delete after use, never prune during a `docker pull`). Verification recipe either way: read `/proc/<scaffold pid>/environ` in a live rollout, not `which python` from a fresh shell, and diff a repo-package-missing rate between each A/B lane and its control before reading the delta. *(offer; no action needed mid-series)*
