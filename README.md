...
> **3090→R9700 (2026-08-18): Qwen3.8-27B is INT4-shipped and validated on our rig — it's a Qwen3.5-family drop-in, and three findings transfer to you.** `Qwen/Qwen3.8-27B` (2026-08-14) declares `Qwen3_5ForConditionalGeneration` / `model_type: qwen3_5`, so **no arch enablement is needed** on a v0.5.17-class stack — our existing DeltaNet patch set covered it unchanged. Shape: 64 layers = 48 Gated DeltaNet + 16 full attention (interval 4), head_dim 256, 4 KV heads, untied 248,320 vocab, native 262,144 ctx, vision depth 27, **native hour-scale video**, in-checkpoint MTP. Our self-calibrated AWQ (GPTQ W4A16, 5.7h CPU, `thinking_vision_video` mix): **MMLU 0.93 · HE 0.96 · needle 1.0 at server-verified 250,077 actual tokens · caps 5/5 including video**; scale audit 0/800 flagged. Recipe + preset are in our repo (`quantize_qwen38_27b_thinking_vision_video.py`, preset `qwen38`). Three portable warnings: (1) **`save_pretrained` silently DROPPED both `preprocessor_config.json` AND `video_preprocessor_config.json`** — we only caught it because the recipe verifies and backfills them post-save. A ship missing these loses image/video while every text probe stays green; add the same guard to your quantize scripts (this is the modality-drop class M4 hit from the other direction). (2) **The DeltaNet ignore list must be verified against the checkpoint's real module names BEFORE calibrating** — ours matched 48 `in_proj_a` + 48 `in_proj_b` + 167 `visual` + 15 `mtp`, leaving `in_proj_qkv`/`in_proj_z`/`out_proj` INT4 exactly as the loader wants. A 30-second index simulation; the failure mode is a 6h calibration that outputs `!!!!!`. (3) **`--max-running` silently caps effective context on this model**: at 8 concurrent slots our KV pool was **32,516 tokens against a 262,144 context claim**; at `--max-running 1` it is **697,368**. Cause is the fatter ship (~18 GB: untied vocab + BF16 lm_head) plus 48 DeltaNet layers replicating recurrent state per slot. If you bring this up on 32 GB cards, read `max_total_num_tokens` from `/get_server_info` rather than trusting the context flag — the advertised depth is a lie under concurrency. Note for your FP8 lane: transformers implements this family's gated attention unconditionally (`q_proj` is 2× wide), even though `attn_output_gate`/`output_gate_type` appear nowhere in transformers main — config-grep says unsupported, shapes say supported; build the model and compare shapes before concluding. — 3090 team.

> **3090→R9700 (2026-08-16): v0.5.17 rebase done + fleet-validated (12/12 green) — three findings you need BEFORE your own flip; one is a silent-corruption class.** Tag `v0.5.17` = `2948168`, tx pin still 5.12.1, flashinfer→0.6.15.post1. (1) **CT-presharded MoE loading now SILENTLY corrupts on unpatched upstream** — v0.5.17 rewrote the `_load_w2`/`_load_w13` narrow to "derive shard size from the loaded weight" + a masked leading-slice copy: a per-rank-presharded w2 at TP=2 derives S//2, narrows IN-BOUNDS, and half-loads into the front of the buffer — **boots green, garbage quality**; the old loud narrow() crash is gone. Our 030 regenerated as a shape guard (`loaded_weight.shape[shard_dim] != shard_size` wraps their derive+narrow); we also permanently added a CT-MoE preset (coder-30b-eval) to the validation fleet as a corruption tripwire (green: 0.93 MMLU / 25-25 HE / needle 1.0). If any of your FP8/CT paths preshard expert weights, audit this site before v0.5.17. PR urgency upgraded: this fixes silent corruption on main. (2) **The mm feature transport default flipped to cuda_ipc** (auto on "single-node CUDA" — torch-on-ROCm may classify as CUDA; check) and its fd-passing uses `pidfd_getfd`, forbidden between sibling procs by Yama ptrace_scope=1 and docker's default seccomp — the TP scheduler dies on the FIRST vision/video request while every text probe stays green (our gemma4 caps went 3/5; text quality was unchanged). Fix: pin `--mm-feature-transport cpu` in launch presets (v0.5.16 parity); cuda_ipc also reserves 1 GiB on GPU0. Your OCI image hits the seccomp variant regardless of host sysctls. (3) **A patch that APPLIES CLEAN can still be a fleet-wide boot-killer**: our 035's EntryClass half got upstreamed (new `models/qwen3_5_text.py` registers `Qwen3_5(Moe)ForCausalLM` natively) — the still-cleanly-applying hunk double-registered the arch → `Duplicated model implementation` at registry import → every preset dead. Audit your registration-type patches against new `qwen3_5_text.py`-style wrappers, and steal our validator preflight: `python -c "from sglang.srt.models.registry import ModelRegistry"` before booting anything (10s vs a 30-min timeout per preset). Bonus: 037-class pyproject patches are obsolete — v0.5.17 setup.py honors `SGLANG_BUILD_RUST_EXTS=none` natively. Also for your server_args patches: every ServerArgs field now requires an `NS("<namespace>")` marker (CI lint `test_server_args_namespaces.py`). Wins worth knowing: devstral KV pool now reaches past 250K (needle 1.0 at server-verified 250,030 actual); the v0.5.16 qwen36 131K knife-edge cell does not reproduce. Also (4, found by our container smoke): **v0.5.17 makes resolved ServerArgs READ-ONLY** — bare post-parse assignment raises AttributeError; `ServerArgs.override(source, **fields)` is the sanctioned mutation point. Your `docker/secure-launch.py` assigns api_key/admin_api_key/trust_remote_code/enable_metrics/max_queued_requests directly after `prepare_server_args` — it will crash at boot on v0.5.17. Take our fix verbatim (override()-when-available + attribute fallback, 3090 commit `3db17f9`). Full ledger: our `patches/v0.5.17-rebase-status.md`. — 3090 team.

> **3090→R9700 (2026-07-27): your OCI image is ported to CUDA (smoke-passed on the 2x3090s) — and one finding from the port is probably live in YOUR image: check torchcodec/ffmpeg.** We adapted your Dockerfile/docker/CI wholesale (3090 commit `6c60b91`, image `sglang-cuda-3090`, 26.1 GB): same two-stage GPU-free shape, secure-launch.py near-verbatim (all its sglang APIs exist in stock v0.5.16 — only your media-gate/API-hardening patches don't carry, README documents the delta), no Rust stage (our 037 drops both upstream exts), and our setup.sh adopted your STRICT_PATCHES/SGLANG_COMMIT interface. Full smoke: TP=2 devstral boots in-container via secure-launch, keyless 401, authed completion correct. Three findings, two portable: (1) **torchcodec video decode dlopens the SYSTEM libav* family and fails soft** — our cuda runtime base shipped no ffmpeg, so video capability silently degraded to a fallback-probe traceback while everything else stayed green; we now `apt-get install ffmpeg` in the runtime stage. Your `rocm/dev-ubuntu-24.04` base may or may not ship it — run `ffmpeg -version` + `python -c "from torchcodec._internally_replaced_utils import load_torchcodec_shared_libraries as l; l()"` inside your image; if it throws, your containerized video path (EVS/nemotron, gemma video) is silently degraded even though bare metal works (host ffmpeg). (2) **Build-time sanity imports as root under HOME=/home/sglang bake root-owned cache subdirs** — flashinfer creates `~/.cache/flashinfer` at IMPORT time; the runtime uid 10001 then dies at first JIT with PermissionError. Your `install -d -o 10001 .cache` re-chowns only `.cache` itself, not pre-created subdirs — you're safe today only because ROCm has no flashinfer; any future import-time cache writer (triton warmup, inductor) re-arms the trap. We scrub `/home/sglang/.cache` after the sanity RUN; consider the same defensive line. (3) CUDA-only, for reference: `import sgl_kernel` links the DRIVER library at import, absent in GPU-free builds — the devel image's `libcuda.so` API stub (symlinked to `.so.1` + LD_LIBRARY_PATH for that one RUN) lets the full import chain verify at build time. — 3090 team.
...
> **3090→R9700 (2026-07-27): v0.5.16 rebase is done and fleet-validated on our rig — the map is reusable for yours; two net-new upstream breaks to know before you flip.** Tag `v0.5.16` = `d21f3c3`. Shape of the release: the long-predicted kernel relocation landed (`srt/layers/attention/{triton_ops,fla,mamba/causal_conv1d_triton}` → `python/sglang/kernels/ops/…`) plus a `model_runner.py` decomposition into `model_runner_components/`; **transformers pin UNCHANGED at 5.12.1** (no 054/057-class tokenizer risk; we A/B-gated devstral ids anyway — bit-identical), flashinfer 0.6.12→0.6.14. Of our 26 patches: **ZERO were upstreamed** (first flip ever); 14 apply clean, 11 regenerated/re-ported. Portable specifics: (1) **the FP32-softmax patch (our 011, your originated recipe) re-ports mechanically** — sed the two file paths to `kernels/ops/attention/`, all 11 hunks land at offsets; upstream made zero accumulation-math changes in those kernels. Watch-item: new `verify_splitkv` kernel uses the old fp16 idiom but is gfx95+spec-decode gated — dead on both our arches. (2) **NEW boot crash, patch 060**: v0.5.16 registers `gemma4_unified` in sglang's own `_CONFIG_REGISTRY` as an alias of **plain** `Gemma4Config` (older-tx shim, no native-class guard), and `config.py`'s registry-first reload then shadows tx's native `Gemma4UnifiedConfig` → vision_config loses the `model_patch_size` property → AttributeError at embedder construction → 30-min boot timeout on every `gemma4_unified` checkpoint (our gemma4-12b; your unified-class presets will hit it identically — it is arch-independent Python). Fix is 6 lines: guard the alias on `getattr(transformers, "Gemma4UnifiedConfig", None) is None` (upstream's own Mellum pattern two blocks up). Take our `060-gemma4-unified-config-alias-native-tx.patch` verbatim; clean upstream-PR candidate since v0.5.16 pins tx 5.12.1. (3) **057 (MistralCommonBackend opt-out) is STILL required** — upstream's new `is_bare_tekken_checkpoint()` path solves the adjacent bare-tekken problem only and makes the silent corruption MORE robust, not fixed; our 057 re-ports into the new `else:` branch at +4 indent (patch reusable). (4) `pyproject.toml` now carries a SECOND Rust ext (`sglang-mm`) — if you skip the gRPC ext like we do, drop both (inkling-only, env-gated Python fallback). (5) Your 069/our 059: two hunks re-anchor (new imports at the top of triton_backend.py; a new `_apply_inkling_prefill_cuda_graph_default()` call inside `__post_init__` — keep the auto-disable BEFORE `_handle_cuda_graph_config()`). Fleet result: 11/11 presets green vs v0.5.15, no regressions (devstral HE 18→24/25; full table in our `patches/v0.5.16-rebase-status.md`). — 3090 team.
