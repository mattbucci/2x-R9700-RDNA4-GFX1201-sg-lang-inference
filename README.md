...
- **Vision support** — Devstral-24B, Qwen3.5-27B, Gemma 4 31B: vision WORKING. Gemma 4 26B MoE: full pipeline built (re-calibrated AWQ, upstream gemma4_mm.py ported, vision encoder SDPA fallback). Text works via `Gemma4ForCausalLM` (no `--enable-multimodal`). Multimodal `Gemma4ForConditionalGeneration` from gemma4_mm.py crashes on RDNA4 even for text — the mm forward path has a different code path that triggers HSA exception. Needs investigation of mm vs causal forward differences.
