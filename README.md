...
- **Vision support** — Devstral-24B, Qwen3.5-27B, Gemma 4 31B: vision WORKING. Gemma 4 26B MoE: text works via `Gemma4ForCausalLM` (30 tok/s). Re-calibrated AWQ-v3 text also works. Vision blocked by `gemma4_mm.py` forward path crash: the mm path has `positions += 1` and `per_layer_inputs` handling that triggers HSA exception even for text-only requests. Upstream multimodal files ported (gemma4_mm.py, gemma4_vision.py, gemma4_audio.py + SDPA fallback for vision encoder) but mm forward needs debugging.
