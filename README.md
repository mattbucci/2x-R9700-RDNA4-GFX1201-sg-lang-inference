...
- **Vision support** — Devstral-24B, Qwen3.5-27B, Gemma 4 31B: vision WORKING. Gemma 4 26B MoE: text fixed (torch_native attn), vision DEGRADED — BF16 vision weights restored but MoE expert routing misaligned for vision tokens (answers "Blue" for all colors). Root cause: GPTQ forced-routing calibration used text-only data, so experts were calibrated for text activation distributions. Vision tokens route differently through MoE, hitting poorly-calibrated experts. Fix path: re-calibrate with multimodal data using [llm-compressor](https://github.com/vllm-project/llm-compressor) (see [gemma3 example](https://github.com/vllm-project/llm-compressor/blob/main/examples/multimodal_vision/gemma3_example.py)) or [MBQ](https://github.com/thu-nics/MBQ). Calibration datasets: [flickr30k](https://huggingface.co/datasets/flickr30k) (image-text, used by llm-compressor), [COCO](https://cocodataset.org/) + [ShareGPT4V](https://huggingface.co/datasets/Lin-Chen/ShareGPT4V) (used by MBQ interleave format).
